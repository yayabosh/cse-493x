...
from chapter9 import EVENT_DISPATCH_CODE
...
def request(url, top_level_url, payload=None, referrer_policy=None):
    ...
    if scheme == "https":
        ...
        try:
            ...
            headers["is_https"] = True
        except ssl.CertificateError:
            return {}, "<!doctype html>Secure Connection Failed"
...
class Tab:
    ...
    def load(self, url, body=None):
        headers, body = request(url, self.url, body, self.referrer_policy)
        self.scroll = 0
        self.url = url
        self.history.append(url)

        if headers.get("is_https", False):
            self.is_https = True
        ...
        for script in scripts:
            ...
            if not self.allowed_request(script_url):
                ...
                continue
            header, body = request(
                script_url, url, referrer_policy=self.referrer_policy
            )
            try:
                self.js.run(body)
        ...
        for link in links:
            ...
            try:
                header, body = request(
                    style_url, url, referrer_policy=self.referrer_policy
                )
            except:
                continue
    ...
    def draw(self):
        ...
        if self.focus == "address bar":
            ...
            self.canvas.create_text(
                ...
                55,
                anchor="nw",
                text=url + "\N{lock}" if self.tabs[self.active_tab].is_https else "",
                font=buttonfont,
                fill="black",
            )

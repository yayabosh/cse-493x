    ...
    def body(self):
        ...
        while self.i < len(self.s) and self.s[self.i] != "}":
            try:
                ...
                if prop == "font":
                    vals = val.split()
                    pairs["font-style"] = vals[0]
                    pairs["font-weight"] = vals[1]
                    pairs["font-size"] = vals[2]
                    pairs["font-family"] = vals[3]
                else:
...
class ClassSelector:
    ...
    # Returns true if the node has the class attribute and the class attribute
    # contains the class name.
    def matches(self, node):
        return (
            isinstance(node, Element)
            and self.classname in node.attributes.get("class", "").split()
        )
...
class BlockLayout:
    ...
    def layout(self):
        if self.node.style.get("width", "auto") == "auto":  # auto width
            self.width = self.parent.width
        else:
            self.width = float(self.node.style["width"])
...
class DrawText:
    ...
    def __repr__(self):
        return f"DrawText(top={self.top} left={self.left} bottom={self.bottom} text={self.text} font={self.font})"

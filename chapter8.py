...
from chapter5 import BLOCK_ELEMENTS, DrawRect
from chapter6 import CSSParser, TagSelector, DescendantSelector
from chapter6 import INHERITED_PROPERTIES, style, cascade_priority, compute_style
from chapter6 import DrawText, resolve_url, tree_to_list
from chapter7 import LineLayout, TextLayout, CHROME_PX
...
class InputLayout:
    ...
    def paint(self, display_list):
        ...
        if self.node.tag == "input":
            if type == "checkbox":
                text = self.node.attributes.get("value", "on")
            else:
...
class BlockLayout:
    ...
    def __repr__(self):
        mode = layout_mode(self.node) == "block" and "Block" or "Inline"
        return "{}Layout(x={}, y={}, width={}, height={})".format(
            mode, self.x, self.y, self.width, self.height
        )
...
class Tab:
    ...
    def draw(self, canvas):
        ...
        if self.focus:
            obj = [
                ...
                if obj.node == self.focus and isinstance(obj, InputLayout)
            ][0]
        ...
        while elt:
            if isinstance(elt, Text):
                ...
                if elt.attributes.get("type") == "checkbox":
                    on = elt.attributes.get("value") == "on"
                    elt.attributes["value"] = "off" if on else "on"
                else:
    ...
    def tab(self):
        if self.focus and self.focus.tag == "input":
            ...
            children = tree_to_list(curr_node, [])
            inputs = [
                child
                for child in children
                if isinstance(child, Element) and child.tag == "input"
            ]
            curr_index = inputs.index(self.focus)
            # Compute the next element to focus on
            next_index = (curr_index + 1) % len(inputs)
            # Shift the focus there
            self.focus = inputs[next_index]
    ...
    def submit_form(self, elt):
        ...
        if elt.attributes.get("method") == "POST":
            # Send a POST request
            self.load(url, body)
        else:
            # GET request, append body with question mark
            self.load(url + "?" + body)
...
class Browser:
    ...
    def draw(self):
        ...
        tabfont = get_font(20, "normal", "roman", "Times")
        ...
        buttonfont = get_font(30, "normal", "roman", "Times")

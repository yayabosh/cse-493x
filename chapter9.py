...
from chapter8 import request, layout_mode
from chapter8 import DocumentLayout, BlockLayout, InputLayout, INPUT_WIDTH_PX
...
class JSContext:
    ...
    def innerHTML_set(self, handle, s):
        ...
        for child in elt.children:
            for node in tree_to_list(child, []):
                if isinstance(node, Element) and "id" in node.attributes:
                    self.run(
    ...
    # Return a list of integers (handles) which represent the children of the
    # node with the given handle.
    def get_children(self, handle):
        node = self.handle_to_node[handle]
        # We don't want Text tags
        children = [
            element for element in node.children if isinstance(element, Element)
        ]
    ...
    def insert_before(self, parent_handle, new_handle, reference_handle):
        parent_node = self.handle_to_node[parent_handle]
        new_node = self.handle_to_node[new_handle]
        ref_node = self.handle_to_node[reference_handle]
